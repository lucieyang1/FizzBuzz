...
> - *it seems that the type of variables are said before the variable name (ex. `boolean divisibleBy3`)*
> - *there's brackets that contain what happens after `if/else` statements*
> - *after each line, there's a semicolon (;)*
> - *the `for` loop doesn't need a counting variable, it does it by itself? (assuming with `i++`)*
> - *the `for` loop can have a limit!*
> - *`elif` in Python becomes `else if` in Java*
